<template>
  <DialogCompatibilityIssueVue
    ref="dialogCompatibility"
  ></DialogCompatibilityIssueVue>
  <BasicBackground>
    <v-card class="login-container">
      <v-tooltip
        ...
        <template #activator="{ props }">
          <v-icon v-bind="props" class="helpIcon"
      ...
      <v-card-subtitle
        >Lokaler Demo-Login ohne Backend. Voreingestellt sind admin / admin,
        damit du direkt ins Template kommst.</v-card-subtitle
      >
      <v-alert
        class="demo-hint"
        type="info"
        ...
        density="comfortable"
      >
        Dieses Template ist absichtlich mit dem Demo-Login admin / admin
        vorbelegt. Ersetze die Werte später durch deine eigene
        Authentifizierung.
      </v-alert>
      <v-card-text class="form-container">
        <v-form ref="form" v-model="valid" lazy-validation>
          ...
          <v-text-field
            ...
            required
          ></v-text-field>
          <v-btn v-if="!loading" :disabled="!valid" @click="localLogin">
            Login
          </v-btn>
          <v-progress-circular
            v-if="loading"
...
onMounted(() => {
  ...
  const isIframe: boolean =
    router.currentRoute.value.query.iframe?.toString().toLowerCase() === "true";
